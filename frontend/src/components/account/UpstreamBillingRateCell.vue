  ...
  >
    ...
    >
      ...
      <div class="flex min-w-0 items-center gap-0.5 overflow-hidden">
        ...
        <span
          v-if="hasEffectiveRate && statusLabel && snapshot?.status === 'failed'"
          :class="statusClass"
...
const compactMetricLayout = computed(() => locale?.value?.startsWith('zh') ?? false)
...
const cachedRate = computed(() => {
  ...
  return lastDetectedRate.value == null ? '' : `${formatMultiplier(lastDetectedRate.value)}x`
})
